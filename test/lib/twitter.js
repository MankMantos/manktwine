...
describe('the twitter module', () => {
    ...
    it('should invoke GET APIs', async () => {
        ...
        let respose = await twitter.get('/api');
        expect(respose).to.equal('foo');
        axios.get.restore();
    });

    it('should invoke POST APIs', async () => {
        sinon.stub(axios, 'post').resolves({data: 'bar'});
        let respose = await twitter.post('/api', 'stuff');
        expect(respose).to.equal('bar');
        axios.post.restore();
    });
    ...
    it('should reject limit rate errors', async () => {
