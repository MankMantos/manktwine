...
const configure = {
    ...
    async account(name) {
        let creds = new CredentialManager(name);
        let [apiKey, secret] = await creds.getKeyAndSecret('consumer');
        let twitter = new Twitter(apiKey, secret);
        ...
        console.log(`Account "${verifyResponse['screen_name']}" successful added`);
